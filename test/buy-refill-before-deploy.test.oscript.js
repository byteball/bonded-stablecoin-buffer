...
describe('Buy T2 through a buffer AA that is refilled before being deployed', function () {
	this.timeout(120000)
	...
	it('Calculate the agent address but do not deploy it yet', async () => {
		this.definition = ["autonomous agent", {
		...
		this.buffer_aa = objectHash.getChash160(this.definition);
		expect(this.buffer_aa).to.be.validAddress
	})
	...
	it('Bob sends bytes to buffer agent, the amount is only slightly more than needed', async () => {
		...
		// there is no response because the address is not an AA yet
	})


	it('Bob deploys the buffer agent', async () => {
		const { address, unit, error } = await this.bob.deployAgent(this.definition)
		expect(error).to.be.null
		expect(unit).to.be.validUnit
		expect(address).to.be.eq(this.buffer_aa)
	})
