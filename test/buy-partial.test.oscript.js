...
describe('Buy T2 through a buffer AA using several partial executions', function () {
	...
	it('Bob sends bytes to buffer agent', async () => {
		this.buffer_amount = 10e9;
	...
	it('Bob triggers execution but it fails because of high fee', async () => {
		this.target_p2 = 1/this.price
		const tokens1 = 0
		const tokens2 = 1e2
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(response.response.error).to.be.eq("one of secondary AAs bounced with error: fee would be " + fee_percent + "%")
		expect(response.bounced).to.be.true
		expect(response.response_unit).to.be.null

		this.buffer_amount += 1e4
	})
	...
	it('Bob triggers execution', async () => {
		...
		const net_buffer_amount = this.buffer_amount + 1e4 - 955
	...
	//	console.log(JSON.stringify(unitObj, null, '\t'))
		...
		this.buffer_amount = remaining_buffer_amount
	})

	it('Bob buys T1 to fix the price', async () => {
		this.target_p2 = 1/this.price
		const tokens1 = 0.002e9
		const tokens2 = 0
		const { amount, fee, fee_percent, reward } = this.buy(tokens1, tokens2)

		const { unit, error } = await this.bob.triggerAaWithData({
			toAddress: this.curve_aa,
			amount: amount - reward + 1000,
			data: {
				tokens1: tokens1,
			},
			spend_unconfirmed: 'all',
	...
	it('Bob triggers another execution', async () => {
		this.target_p2 = 1/this.price
		const tokens1 = 0
		const tokens2 = 1e2
		const { amount, fee, fee_percent } = this.buy(tokens1, tokens2)
